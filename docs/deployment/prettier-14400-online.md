# Prettier #14400：在线采样与原始轨迹准备手册

本文保留早期在线 agent 和手工 replay 的完整操作步骤。当前自动实验直接冷启动固定任务镜像，不要求安装 Claude Code；简明任务介绍见 [workload 文档](../workload/workload-prettier-14400.md)，当前运行入口见[执行与验收](experiments.md)。下文版本、模型名和原机器路径是历史配置记录，重新在线采样时应单独核验并归档实际版本。
